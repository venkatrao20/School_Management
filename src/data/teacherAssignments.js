...
// Full staff record (email, phone, date of joining, role, etc.) for the
// "Teacher / Staff Profiles" feature — the logged-in teacher's own record,
// looked up the same way View Records does: uploaded data first (if any
// admin has uploaded/edited staff records), falling back to the seeded
// sample data so the profile page always has something to show.
export function getStaffProfile(staffId) {
  if (!staffId) return null;
  const uploaded = getRecords("staff");
  const pool = uploaded.length > 0 ? uploaded : SEED_DATA.staff;
  return pool.find((s) => s.staffId === staffId) || null;
}
