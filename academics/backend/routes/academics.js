...
router.get('/teachers', async (req, res) => {
    const rows = await db.prepare('SELECT * FROM teachers ORDER BY name').all();
    ok(res, rows);
});

router.get('/teachers/:id', async (req, res) => {
    const row = await db.prepare('SELECT * FROM teachers WHERE id = ?').get(req.params.id);
...
router.post('/teachers', async (req, res) => {
    ...
    try {
        ...
        created(res, await db.prepare('SELECT * FROM teachers WHERE id = ?').get(info.lastInsertRowid));
    } catch (e) { fail(res, 400, e.message); }
});

router.put('/teachers/:id', async (req, res) => {
    const existing = await db.prepare('SELECT * FROM teachers WHERE id = ?').get(req.params.id);
    if (!existing) return fail(res, 404, 'Teacher not found');
    const merged = { ...existing, ...req.body };
    await db.prepare(`UPDATE teachers SET name=@name, email=@email, phone=@phone, qualification=@qualification,
    ...
    ok(res, await db.prepare('SELECT * FROM teachers WHERE id = ?').get(req.params.id));
});

router.delete('/teachers/:id', async (req, res) => {
    await db.prepare('DELETE FROM teachers WHERE id = ?').run(req.params.id);
    ok(res, { deleted: true });
});
...
========================================================= */
router.get('/classes', async (req, res) => ok(res, await db.prepare('SELECT * FROM classes ORDER BY name, section').all()));
router.post('/classes', async (req, res) => {
    const { name, section } = req.body;
    if (!name || !section) return fail(res, 400, 'name and section required');
    const info = await db.prepare('INSERT INTO classes (name, section) VALUES (?, ?)').run(name, section);
    created(res, await db.prepare('SELECT * FROM classes WHERE id = ?').get(info.lastInsertRowid));
});

router.get('/subjects', async (req, res) => ok(res, await db.prepare('SELECT * FROM subjects ORDER BY name').all()));
router.post('/subjects', async (req, res) => {
    const { name, code } = req.body;
    if (!name) return fail(res, 400, 'name required');
    const info = await db.prepare('INSERT INTO subjects (name, code) VALUES (?, ?)').run(name, code || null);
    created(res, await db.prepare('SELECT * FROM subjects WHERE id = ?').get(info.lastInsertRowid));
});

router.get('/students', async (req, res) => {
    const { class_id } = req.query;
    let rows;
    if (class_id) rows = await db.prepare('SELECT * FROM students WHERE class_id = ? ORDER BY roll_no').all(class_id);
    else rows = await db.prepare('SELECT * FROM students ORDER BY name').all();
    ok(res, rows);
});
router.post('/students', async (req, res) => {
    ...
    const info = await db.prepare('INSERT INTO students (admission_no, name, class_id, roll_no) VALUES (?, ?, ?, ?)').run(admission_no, name, class_id, roll_no || null);
    created(res, await db.prepare('SELECT * FROM students WHERE id = ?').get(info.lastInsertRowid));
});
...
router.post('/allocations', async (req, res) => {
    ...
    const info = await db.prepare(`INSERT INTO teacher_class_allocation (teacher_id, class_id, subject_id, is_class_teacher, academic_year)
        VALUES (?, ?, ?, ?, ?)`).run(teacher_id, class_id, subject_id, is_class_teacher ? 1 : 0, academic_year || '2026-2027');
    created(res, await db.prepare('SELECT * FROM teacher_class_allocation WHERE id = ?').get(info.lastInsertRowid));
});

router.delete('/allocations/:id', async (req, res) => {
    await db.prepare('DELETE FROM teacher_class_allocation WHERE id = ?').run(req.params.id);
    ok(res, { deleted: true });
});
...
router.post('/timetable', async (req, res) => {
    ...
    const info = await db.prepare(`INSERT INTO timetable (class_id, subject_id, teacher_id, day_of_week, period_no, start_time, end_time, room)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(class_id, subject_id, teacher_id, day_of_week, period_no, start_time, end_time, room || null);
    created(res, await db.prepare('SELECT * FROM timetable WHERE id = ?').get(info.lastInsertRowid));
});

router.delete('/timetable/:id', async (req, res) => {
    await db.prepare('DELETE FROM timetable WHERE id = ?').run(req.params.id);
    ok(res, { deleted: true });
});
...
router.post('/homework', async (req, res) => {
    ...
    const info = await db.prepare(`INSERT INTO homework (class_id, subject_id, teacher_id, title, description, assigned_date, due_date, attachment_url, status)
    ...
    created(res, await db.prepare('SELECT * FROM homework WHERE id = ?').get(info.lastInsertRowid));
});

router.put('/homework/:id', async (req, res) => {
    const existing = await db.prepare('SELECT * FROM homework WHERE id = ?').get(req.params.id);
    if (!existing) return fail(res, 404, 'Homework not found');
    const merged = { ...existing, ...req.body };
    await db.prepare(`UPDATE homework SET title=@title, description=@description, due_date=@due_date, attachment_url=@attachment_url, status=@status WHERE id=@id`).run(merged);
    ok(res, await db.prepare('SELECT * FROM homework WHERE id = ?').get(req.params.id));
});

router.delete('/homework/:id', async (req, res) => {
    await db.prepare('DELETE FROM homework WHERE id = ?').run(req.params.id);
    ok(res, { deleted: true });
});
...
router.post('/syllabus', async (req, res) => {
    ...
    const info = await db.prepare(`INSERT INTO syllabus (class_id, subject_id, teacher_id, unit_title, topics, planned_start_date, planned_end_date, completion_status, completion_percent)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
    ...
    created(res, await db.prepare('SELECT * FROM syllabus WHERE id = ?').get(info.lastInsertRowid));
});

router.put('/syllabus/:id', async (req, res) => {
    const existing = await db.prepare('SELECT * FROM syllabus WHERE id = ?').get(req.params.id);
    if (!existing) return fail(res, 404, 'Syllabus entry not found');
    const merged = { ...existing, ...req.body };
    await db.prepare(`UPDATE syllabus SET unit_title=@unit_title, topics=@topics, planned_start_date=@planned_start_date,
    ...
    ok(res, await db.prepare('SELECT * FROM syllabus WHERE id = ?').get(req.params.id));
});

router.delete('/syllabus/:id', async (req, res) => {
    await db.prepare('DELETE FROM syllabus WHERE id = ?').run(req.params.id);
    ok(res, { deleted: true });
});
...
router.get('/calendar', async (req, res) => {
    ok(res, await db.prepare('SELECT * FROM academic_calendar ORDER BY start_date').all());
});
...
router.post('/calendar', async (req, res) => {
    ...
    const info = await db.prepare(`INSERT INTO academic_calendar (title, event_type, start_date, end_date, description) VALUES (?, ?, ?, ?, ?)`)
        .run(title, event_type || null, start_date, end_date || start_date, description || null);
    created(res, await db.prepare('SELECT * FROM academic_calendar WHERE id = ?').get(info.lastInsertRowid));
});

router.delete('/calendar/:id', async (req, res) => {
    await db.prepare('DELETE FROM academic_calendar WHERE id = ?').run(req.params.id);
    ok(res, { deleted: true });
});
...
router.get('/dashboard', async (req, res) => {
    const totalTeachers = (await db.prepare('SELECT COUNT(*) c FROM teachers WHERE status = "Active"').get()).c;
    const totalStudents = (await db.prepare('SELECT COUNT(*) c FROM students').get()).c;
    const totalClasses = (await db.prepare('SELECT COUNT(*) c FROM classes').get()).c;
    const totalHomeworkThisWeek = (await db.prepare("SELECT COUNT(*) c FROM homework WHERE assigned_date >= date('now','-7 day')").get()).c;
    const syllabusProgress = await db.prepare(`
    ...
    `).all();
    const upcomingEvents = await db.prepare(`SELECT * FROM academic_calendar WHERE start_date >= date('now') ORDER BY start_date LIMIT 5`).all();
    const todayAttendanceSummary = await db.prepare(`
...
========================================================= */
router.get('/academic-info/:studentId', async (req, res) => {
    ...
    const homeworkAssigned = (await db.prepare(`
